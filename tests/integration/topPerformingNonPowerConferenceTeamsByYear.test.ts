...
import { GET } from "@/app/api/teams/top-performers/non-power-conference/[yearId]/route";
...
vi.mock("@/application/useCases", () => ({
  getTopPerformingNonPowerConferenceTeamsByYear: (...args: unknown[]) =>
    mockGetTopPerformingNonPowerConferenceTeamsByYear(...args),
}));

function createRequest(yearId: string): NextRequest {
  return new NextRequest(
    `http://localhost:3000/api/teams/top-performers/non-power-conference/${yearId}`,
    { method: "GET" },
  );
}
...
describe("GET /api/teams/top-performers/non-power-conference/[yearId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe("successful requests", () => {
    it("should return 200 with top non power conference teams for a valid year id", async () => {
      const mockTeams = [
  ...
  describe("error handling", () => {
      ...
      expect(json).toEqual(
        expect.objectContaining({
          ...
          error: expect.objectContaining({
            ...
            path: "/api/teams/top-performers/non-power-conference/test-year-id",
          }),
        }),
      );
    });
    ...
    it("should return 500 when no team performance records are found", async () => {
      mockGetTopPerformingNonPowerConferenceTeamsByYear.mockRejectedValue(
        new Error(
          "No top performing teams found for non-power conferences and year_id: missing-year-id",
        ),
      );
