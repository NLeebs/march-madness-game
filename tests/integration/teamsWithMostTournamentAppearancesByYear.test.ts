...
vi.mock("@/application/useCases", () => ({
  getTeamsWithMostTournamentAppearancesByYear: (...args: unknown[]) =>
    mockGetTeamsWithMostTournamentAppearancesByYear(...args),
}));

function createRequest(yearId: string): NextRequest {
  return new NextRequest(
    `http://localhost:3000/api/teams/appearances/${yearId}`,
    { method: "GET" },
  );
}
...
describe("GET /api/teams/appearances/[yearId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe("successful requests", () => {
    it("should return 200 with teams with most tournament appearances for a valid year id", async () => {
      const mockTeams = [
      ...
      mockGetTeamsWithMostTournamentAppearancesByYear.mockResolvedValue(mockTeams);
      ...
      expect(json).toEqual(
        expect.objectContaining({
          ...
          error: expect.objectContaining({
            ...
            path: "/api/teams/appearances/test-year-id",
          }),
        }),
      );
    });

    it("should return 500 when no appearance records are found", async () => {
      mockGetTeamsWithMostTournamentAppearancesByYear.mockRejectedValue(
        new Error(
          "No teams with most tournament appearances found for year_id: missing-year-id",
        ),
      );
      ...
      expect(json.error.message).toContain(
        "No teams with most tournament appearances found",
      );
    });
  });
