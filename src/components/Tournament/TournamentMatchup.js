...
function TournamentMatchup(props) {
    ...
    const matchupElements = props.matchup.map((teamObj, i) => {
        return (
        <div key={i} onClick={appState.tournament && teamSelectionClickHandler} value={i} team={teamObj.team} seed={teamObj.seed} className="team-selection flex items-center px-4 h-14 border-2 border-slate-100 rounded-md cursor-pointer">
