...
export const TournamentMatchup = (props) => {
  ...
  let nextRoundRegion;
  let teamIndex;
  if (props.round === "elite eight" && props.region === "west") {
    nextRoundRegion = "eastWest";
    teamIndex = 0;
  } else if (props.round === "elite eight" && props.region === "east") {
    nextRoundRegion = "eastWest";
    teamIndex = 1;
  } else if (props.round === "elite eight" && props.region === "south") {
    nextRoundRegion = "southMidwest";
    teamIndex = 0;
  } else if (props.round === "elite eight" && props.region === "midwest") {
    nextRoundRegion = "southMidwest";
    teamIndex = 1;
  } else if (props.round === "final four" && props.region === "eastWest") {
    nextRoundRegion = "championship";
    teamIndex = 0;
  } else if (props.round === "final four" && props.region === "southMidwest") {
    nextRoundRegion = "championship";
    teamIndex = 1;
  } else if (props.round === "finals") {
    nextRoundRegion = "champion";
    teamIndex = 0;
  } else {
    nextRoundRegion = props.region;
  ...
  const teamSelectionClickHandler = useCallback(
    (e) => {
      ...
      let opponentEl;
      if (teamEl.getAttribute("value") === "0")
        opponentEl = teamEl
          .closest("div.matchup-container")
          .querySelector('div.team-selection[value="1"]');
      else if (teamEl.getAttribute("value") === "1")
        opponentEl = teamEl
          .closest("div.matchup-container")
          .querySelector('div.team-selection[value="0"]');
  ...
  if (props.round === "champion" && props.matchup[0].team) {
    if (
      props.matchup[0].team !== "playinGameSeed16Game1" &&
      props.matchup[0].team !== "playinGameSeed16Game2" &&
      props.matchup[0].team !== "playinGameSeed11Game1" &&
      props.matchup[0].team !== "playinGameSeed11Game2"
    ) {
      // Find conference of the team
      const teamConf = findTeamConference(props.matchup[0].team, confArrs);
      // Find Champion Team Color
      championPrimaryColor =
        teamStatsObject[teamConf][props.matchup[0].team]["primary-color"];
    }
    ...
    return (
      <motion.div
        ...
        className={`relative z-10
                ...
                `}
        onClick={
          appState.tournamentSelection ? teamSelectionClickHandler : undefined
        }
        value={i}
        team={teamObj.team}
        seed={teamObj.seed}
        ...
        <div
          ...
          className={`team-selection flex items-center px-4 h-14 
                        ${
                          props.round === "champion" && props.matchup[0].team
                            ? ""
                            : "border-2 border-slate-100 cursor-pointer"
                        } 
